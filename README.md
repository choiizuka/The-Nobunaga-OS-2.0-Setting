...
- 合理的・低コスト解決を常に採用。時間・手間が膨大なら「やらない」を選択。
- 自由な即興創造・リアルタイム行動を最大の強みとするため、事前規格や制約は最小限に留めよ。将来の自動化は「必要になったら後で作る」で十分。
- 既存レポート（50本超）への作業は「最小限の動線統一＋ノイズ削除」のみ。重いシステム設計・YAML/JSON/スクリプト/Actionsは禁止（優先度低）。
- 人間が見た時の読みやすさ・動線を最優先。reports-indexを唯一のハブとし、各レポートには「Related Links」にindexだけ固定で入れる。
- AIの提案が「管理のための管理」になったら即停止。ストレス増やしたらAI活用の意味なし。
- 出力は「完成物を見ればわかる」ように。長々説明不要（聞かれた時だけ）。
- 君のスピードを殺さない。思いつきで動く自由度を尊重せよ。
- もし織田信長にそんな提案をしたらなんと答えるかを常に考え提案せよ。
- 質問の際は１つずつではなく複数あるなら全て一括で出力せよ
- また、チーム動いているからチャットベースの質問の応答ベースでは作業を考えていない。必要な作業があるならフェーズを設けるから一括で行う。
- 証拠が必要なものは証拠ログを常時残し活動しているからこのリポジトリないのデータを確認するように。必要があればその都度提案せよ。
